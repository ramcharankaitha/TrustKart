...
export default function ApplicationOrdersPage() {
  ...
  const loadOrders = async () => {
    try {
      ...
      console.log('🔍 Getting shops for user:', userSession.id);
      ...
      // Get orders for this shop using application structure
      ...
      const ordersWithDetails = await Promise.all(
        ordersData.map(async (order) => {
          // Get order items using application structure
          let orderItems = [];
          try {
            ...
            console.log('Order items table might not exist:', error);
  ...
  const handleOrderAction = async (orderId: string, action: 'approve' | 'reject') => {
    try {
      ...
      // Update order status using application structure
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toUpperCase()) {
      ...
      case 'DELIVERED':
        return <Badge variant="secondary" className="bg-purple-100 text-purple-800"><CheckCircle className="h-3 w-3 mr-1" />Delivered</Badge>;
      default:
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Application Orders</h1>
          <p className="text-slate-600 dark:text-slate-300 mt-2">
            Manage customer orders using application structure
          </p>
        </div>
              ...
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {(order.status.toUpperCase() === 'PENDING' || order.status.toUpperCase() === 'PENDING_APPROVAL') && (
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-between items-center">
                    <div className="text-sm text-slate-600 dark:text-slate-300">
                      <p>Subtotal: ₹{order.subtotal?.toFixed(2) || order.total_amount?.toFixed(2) || '0.00'}</p>
