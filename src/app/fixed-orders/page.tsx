...
export default function FixedOrdersPage() {
  ...
  const loadOrders = async () => {
    try {
      ...
      if (userSession.role !== 'shopkeeper') {
        console.error('❌ User is not a shopkeeper');
        setError('You must be logged in as a shopkeeper to view orders.');
        toast({
          variant: "destructive",
          title: "Access Denied",
          description: "Only shopkeepers can view orders.",
        });
        return;
      }
      ...
      // Get shops owned by this user - DIRECT QUERY
      ...
      // Get orders for this shop - DIRECT QUERY
      ...
      // Get order items for each order - DIRECT QUERY
      ...
      const ordersWithItems = await Promise.all(
        ordersData.map(async (order) => {
          ...
          // Get product details for each item - DIRECT QUERY
      ...
      // Get customer details for each order - DIRECT QUERY
