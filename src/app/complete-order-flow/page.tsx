...
export default function CompleteOrderFlowPage() {
        ...
        ordersData.map(async (order) => {
          ...
          const itemsWithProducts = await Promise.all(
            (orderItems || []).map(async (item) => {
              ...
              return {
                ...item,
                product: product || { name: 'Unknown Product', price: item.unit_price, image_url: null }
              };
            })
          );
  ...
  const handleOrderApproval = async (orderId: string) => {
    try {
      ...
      // Update all order items to APPROVED
      ...
      if (itemsError) {
        console.error('❌ Error approving order items:', itemsError);
      }
  ...
  const handleOrderRejection = async (orderId: string, reason: string) => {
    try {
      ...
      // Update all order items to REJECTED
      const { error: itemsError } = await supabase
        .from('order_items')
        .update({ 
          approval_status: 'REJECTED',
      ...
      if (itemsError) {
        console.error('❌ Error rejecting order items:', itemsError);
      }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Complete Order Flow</h1>
