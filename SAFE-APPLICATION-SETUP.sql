...
-- ==============================================
-- 5. CHECK PRODUCTS TABLE STRUCTURE
-- ==============================================
...
-- ==============================================
-- 6. INSERT TEST DATA USING EXISTING USERS AND SAFE PRODUCT INSERTION
-- ==============================================
...
BEGIN
    ...
    RAISE NOTICE 'Products table columns: final_price=%, stock_quantity=%, quantity=%', has_final_price, has_stock_quantity, has_quantity;
    ...
    ('test_shop_safe', shopkeeper_id, 'Safe Test Store', 'Fresh vegetables and groceries', 'grocery', '456 Safe Street', '9876543211', 'safe@test.com', 'APPROVED')
    ON CONFLICT (id) DO NOTHING
    RETURNING id INTO shop_id;
    
    -- If shop already exists, get its ID
    IF shop_id IS NULL THEN
        SELECT id INTO shop_id FROM shops WHERE id = 'test_shop_safe';
    END IF;
    
    RAISE NOTICE 'Using shop_id: %', shop_id;
    
    -- Insert test products using only existing columns
    IF has_final_price AND has_stock_quantity THEN
        -- Use final_price and stock_quantity
        INSERT INTO products (id, shop_id, name, description, price, final_price, stock_quantity) VALUES
        ('test_prod_safe_1', shop_id, 'Fresh Tomatoes', 'Fresh red tomatoes', 50.00, 45.00, 100),
        ('test_prod_safe_2', shop_id, 'Organic Spinach', 'Fresh organic spinach', 30.00, 30.00, 50)
        ON CONFLICT (id) DO NOTHING;
        RAISE NOTICE 'Products inserted with final_price and stock_quantity';
    ELSIF has_quantity THEN
        -- Use quantity only
        INSERT INTO products (id, shop_id, name, description, price, quantity) VALUES
        ('test_prod_safe_1', shop_id, 'Fresh Tomatoes', 'Fresh red tomatoes', 50.00, 100),
        ('test_prod_safe_2', shop_id, 'Organic Spinach', 'Fresh organic spinach', 30.00, 50)
        ON CONFLICT (id) DO NOTHING;
        RAISE NOTICE 'Products inserted with quantity only';
    ELSE
        -- Use only basic columns
        INSERT INTO products (id, shop_id, name, description, price) VALUES
        ('test_prod_safe_1', shop_id, 'Fresh Tomatoes', 'Fresh red tomatoes', 50.00),
        ('test_prod_safe_2', shop_id, 'Organic Spinach', 'Fresh organic spinach', 30.00)
        ON CONFLICT (id) DO NOTHING;
        RAISE NOTICE 'Products inserted with basic columns only';
    END IF;
    ...
    ('test_order_safe', customer_id, shop_id, 'PENDING', 75.00, 70.00, '123 Safe Street', '9876543210', 'Safe test order')
    ON CONFLICT (id) DO NOTHING
    RETURNING id INTO order_id;
    
    -- If order already exists, get its ID
    IF order_id IS NULL THEN
        SELECT id INTO order_id FROM orders WHERE id = 'test_order_safe';
    END IF;
    ...
    ('test_item_safe_1', order_id, 'test_prod_safe_1', 1, 45.00, 'PENDING'),
    ('test_item_safe_2', order_id, 'test_prod_safe_2', 1, 30.00, 'PENDING')
    ON CONFLICT (id) DO NOTHING;
    
    RAISE NOTICE 'Safe test data inserted successfully!';
END $$;

-- ==============================================
-- VERIFICATION
-- ==============================================
SELECT 'Safe application-based order system setup completed!' as status;
